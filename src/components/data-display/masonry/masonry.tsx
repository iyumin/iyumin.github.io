...
import { IMasonryItem, MasonryProps } from '.';
...
const Masonry = React.forwardRef((props: MasonryProps, ref: React.RefObject<HTMLDivElement>) => {
  ...
  const handlePreview = (e: React.MouseEvent<HTMLElement>, item: IMasonryItem) => {
    onPreview(e, item);
  };
  ...
  // each item style
  const getItemStyle = (item: IMasonryItem, index: number) => {
  ...
  const renderItem = (item: IMasonryItem, index: number) => {
